...
sudo chown -R nemke82:nemke2 /home/nemke82/.config/composer;
cd /home/nemke82 &&
composer config -g -a http-basic.repo.magento.com 64229a8ef905329a184da4f174597d25 a0df0bec06011c7f1e8ea8833ca7661e &&
composer create-project --no-interaction --no-progress --repository-url=https://repo.magento.com/ magento/project-community-edition=2.4.5-p2 magento2
cd magento2 && cp -avr .* /home/nemke82;
cd /home/nemke82 && rm -r -f magento2;
mysql -e 'create database nemanja;';
url="https://8002--main--nemanja-test--nemke82--5k0l81l79elbs.pit-1.try.coder.app/"

mysql -u root -pnem4540 -e 'create database nemanja;' &&
cd /home/nemke82 && composer install -n && php bin/magento setup:install --db-name='nemanja' --db-user='root' --db-password='nem4540' --base-url=$url --backend-frontname='admin' --admin-user='admin' --admin-password='adm4540' --admin-email='ne@nemanja.io' --admin-firstname='Nemanja' --admin-lastname='Djuric' --use-rewrites='1' --use-secure='1' --base-url-secure=$url --use-secure-admin='1' --language='en_US' --db-host='127.0.0.1' --cleanup-database --timezone='America/New_York' --currency='USD' --session-save='redis' --amqp-host="127.0.0.1" --amqp-port="5672" --amqp-user="guest" --amqp-password="guest" --amqp-virtualhost="/"
...
php bin/magento config:set web/cookie/cookie_domain ".coder.app" --lock-config &&
